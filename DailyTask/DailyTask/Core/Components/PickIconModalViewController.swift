...
//  DailyTask
//
//  Created by Leonardo Mesquita Alves on 13/10/23.
//
...
class PickIconModalViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let label = LabelComponent(text: String(localized: "Icons"), accessibilityLabel: String(localized: "Icons"),font: .title3)
        label.textLabel.font = UIFont.boldSystemFont(ofSize: UIFont.preferredFont(forTextStyle: .title3).pointSize)
        label.textLabel.textColor = .customLabel
        ...
        titleStack.topAnchor.constraint(equalTo: view.safeAreaLayoutGuide.topAnchor, constant: 16).isActive = true
        titleStack.leadingAnchor.constraint(equalTo: view.safeAreaLayoutGuide.leadingAnchor, constant: 16).isActive = true
        titleStack.trailingAnchor.constraint(equalTo: view.safeAreaLayoutGuide.trailingAnchor, constant: -16 ).isActive = true
        titleStack.bottomAnchor.constraint(equalTo: mainStack.topAnchor, constant: -20).isActive = true
        mainStack.topAnchor.constraint(equalTo: titleStack.bottomAnchor, constant: 20).isActive = true
        ...
        firstStack.heightAnchor.constraint(equalTo: imageOneView.widthAnchor).isActive = true
        
        let contentHeight = mainStack.systemLayoutSizeFitting(UIView.layoutFittingCompressedSize).height + 100
...
//        closeButton.centerYAnchor.constraint(equalTo: view.centerYAnchor).isActive = true
    ...
    @objc func delegateTwentyOne(_ sender: UITapGestureRecognizer) {
        delegate?.buttonWasPressed("rectangle.3.group.fill")
        self.dismiss(animated: true, completion: nil)
    }
...
class CustomIconButton: UIButton{
    ...
    let stackView: UIStackView = {
        ...
        sv.layoutMargins = UIEdgeInsets(top: 15, left: 15, bottom: 15, right: 15)
    ...
    init(image: UIImage){
        ...
        self.image.tintColor = UIColor(.iconModal)
        
        addSubview(stackView)
        stackView.addArrangedSubview(self.image)
        backgroundColor = .iconModalBg
